...
interface Promotor { id: number; nombre: string }
...
export default function SupervisorWhatsAppPage() {
  const [stats, setStats] = useState<Stats | null>(null);
  const [loadingStats, setLoadingStats] = useState(true);

  const fetchStats = useCallback(async () => {
    try {
      const res = await fetch("/api/supervisor/whatsapp/stats");
  ...
  const fetchMensajes = useCallback(async (p = 1) => {
    ...
    try {
      ...
      const res = await fetch(`/api/supervisor/whatsapp/mensajes?${params}`);
      if (res.ok) setMensajesData(await res.json());
    } catch { /* ignore */ }
    setLoadingMensajes(false);
  }, [filtroPromotor, filtroEstado, filtroDesde, filtroHasta]);
  ...
  return (
    <div>
      ...
      <span className="text-sm text-slate-400 block mb-5">
        Monitoreo de envios de WhatsApp de tu equipo
      </span>
