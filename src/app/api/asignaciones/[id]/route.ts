...
export async function GET(
  ...
  const clientes = await prismaClientes.clientes.findMany({
    ...
    select: {
      ...
      nivel_salarial: true,
      puesto: true,
      centro_educativo: true,
      clave_cct: true,
      oferta_neta: true,
      oportunidad_total: true,
      oportunidad_real: true,
      dependencia: true,
      antiguedad: true,
      universo: true,
      nomb_unidad_adm: true,
    },
  });
  ...
  const registros = clientes.map((c) => {
    ...
    return {
      ...
      tipo_nomina: c.tipo_nomina,
      nivel_salarial: c.nivel_salarial,
      puesto: c.puesto,
      centro_educativo: c.centro_educativo,
      clave_cct: c.clave_cct,
      oferta_neta: c.oferta_neta,
      oportunidad_total: c.oportunidad_total,
      oportunidad_real: c.oportunidad_real,
      dependencia: c.dependencia,
      antiguedad: c.antiguedad,
      universo: c.universo,
      nomb_unidad_adm: c.nomb_unidad_adm,
    };
  });
